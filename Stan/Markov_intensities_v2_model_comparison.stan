...
generated quantities{
  array[n_conts] matrix[n_states, n_states] q_mat_gen;
  array[n_conts] matrix[n_states, n_states] p_mat_gen;
  for (t in 1 : n_conts){
        for (i in 1 : n_states){
          for (j in 1 : n_states){
            // if parameter is before the diagonal, place in order
            if (j < i){
              q_mat_gen[t, i, j] =  exp(X[conts[t],] * (beta[i, j]  * 3 - log(n_states-1)));
            } 
            // if diagonal, temporarily hold with zero
            if (j == i){
              q_mat_gen[t, i, j] = 0;
            ...
            // if parameter is after the diagonal, place one ahead
            if (j > i){
              q_mat_gen[t, i, j] =  exp(X[conts[t],] * (beta[i, j - 1] * 3 - log(n_states-1)));
            }
          } 
          q_mat_gen[t, i, i] = - sum(q_mat_gen[t, i, 1 : n_states]);
        }
        p_mat_gen[t] = matrix_exp(q_mat_gen[t]);
}
}
